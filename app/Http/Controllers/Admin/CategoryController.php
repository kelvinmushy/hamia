...
class CategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(isset($image)){
            ...
            $slider = Image::make($image)->resize(1600, 480)->save(storage_path('app/public/category/slider/') .$imagename);
            Storage::disk('public')->put('category/slider/'.$imagename, $slider);

            if(!Storage::disk('public')->exists('category/thumb')){
                Storage::disk('public')->makeDirectory('category/thumb');
            }
            $thumb = Image::make($image)->resize(500, 330)->save(storage_path('app/public/category/thumb/') .$imagename);
    ...
    public function update(Request $request, $id)
    {
        ...
        if(isset($image)){
            ...
            $thumb = Image::make($image)->resize(500, 330)->save(storage_path('app/public/category/slider/') .$imagename);
            Storage::disk('public')->put('category/thumb/'.$imagename, $thumb);
        }else{

...
@section('top')
    <style type="text/css">
        ...
        .thumbnails::after {
            ...
            bottom: 0;
            width: 30px;
            background: linear-gradient(to right, transparent, #0b0b0c);
            pointer-events: none;
        }
...
.member-img {
    ...
    width: 120px;
    height: 120px;
    margin: -60px auto 15px; /* Half the height to overlap the header */
    z-index: 2;
}
...
.l-parpl {
    position: relative;
    padding-top: 60px; /* Make space for the profile image */
    text-align: center;
                ...
                <div class="col-lg-12 col-md-6 col-sm-12">
                    <ul class="breadcrumb float-md-left">
                        ...
                        <li class="breadcrumb-item"><a
                                href="/{{@$property->sub_category->slug}}">{{@$property->sub_category->name }}</a></li>
                    ...
                    <div class="property-gallery-container">
                                ...
                                @foreach($property->property_gallery as $key => $image)
                                    <div class="carousel-item {{ $key == 0 ? 'active' : '' }}">
                                        ...
                                        <img src="{{ asset($image->path) }}" class="d-block w-100"
                                            alt="Property image {{ $key + 1 }}" loading="lazy">
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="thumbnail-container">
                            <div class="thumbnails">
                                @foreach($property->property_gallery as $key => $image)
                                    <img src="{{ asset($image->path) }}" class="thumbnail {{ $key == 0 ? 'active' : '' }}"
                                        data-target="#propertyCarousel" data-slide-to="{{ $key }}" alt="Thumbnail {{ $key + 1 }}"
                                        loading="lazy">
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="body">
                            <div class="property-content">
                                <div class="detail">
                                    ...
                                    <h5 class="text-success m-t-0 m-b-0">{{ $property->currency->name }}
                                        {{number_format($property->price, 0) }}</h5>
                                    <p>Payment Term: <span
                                            class="text text-success">{{@$property->property_term->term->name}}</span></p>
                                    <p class="text-muted"><i
                                            class="zmdi zmdi-pin m-r-5"></i>{{@$property->property_location->region->name}},
                                        {{@$property->property_location->name}}, Tanzania</p>
                        ...
                        <div class="body">
                            <div class="form-group">
                                <form id="form-message" data-toggle="validator" enctype="multipart/form-data">
                                    ...
                                    <span id="messageSent" class="text text-success"></span>
...
@section('bot')
    ...
    <script>
        $(document).ready(function () {
            // Initialize carousel with interval false to prevent auto-sliding
            ...
            $('#propertyCarousel').on('slid.bs.carousel', function () {
                ...
                if ($activeThumb.length) {
                    const container = $('.thumbnails')[0];
                    const thumbPos = $activeThumb.position().left;
                    container.scrollLeft = thumbPos - (container.offsetWidth / 2) + ($activeThumb.width() / 2);
                }
            });
            ...
            $(document).on('click', '.thumbnail', function (e) {
                ...
                $('#propertyCarousel').carousel(parseInt(index));
            });
